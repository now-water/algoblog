...
## 풀이 📣
- 정점을 알파벳 한글자로 보고, 간선을 단어로 생각한다. 즉, 단어 하나씩 뜯어보면서 첫글자에서 마지막 글자로 가는 간선이 해당 단어
- graph[첫글자][마지막글자] = 첫글자 시작, 마지막 글자로 끝나는 단어 저장하는 벡터이다.
- adj[첫글자][마지막글자] = 첫글자 시작, 마지막 글자로 끝나는 단어 개수를 저장한다.
- indegree : 들어오는 간선의 수, outdegree : 나가는 간선의 수
- EulerCheck : 오일러 트레일 or 회로인지 체크 
- 나가는거가 들어오는거보다 1개 적은 경우, 해당 정점에서 트레일 시작. 나가는거랑 들어오는거 같은 경우, 나가는 간선이 있는 아무 정점에서 회로 시작
- 최종적으로 획득한 DAG 의 size가 단어의 size + 1인지 확인 => 처음 단어를 시작할 때 시작-끝 글자도 추가하기 때문에 1개 더 많다
- 위상정렬을 위해 DAG를 뒤집으면 오일러 회로/경로 순서가 나온다.
- 그걸로 graph[첫글자][마지막글자] 하나씩 아무거나 꺼내서 출력한다.
