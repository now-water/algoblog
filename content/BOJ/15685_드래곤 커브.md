...
> 처음에 봤을 때 막막했지만, 규칙성을 찾아서 구현하고 보니 쉬웠다. 90도 회전했을 때의 점들을 계속 추가해나가는 것이 관건<br/>
...
<hr/>
1️⃣ 각 좌표별로 세대의 정보를 저장하는 객체를 생성한다(끝점, 좌표리스트) <br/>
2️⃣ 세대를 진화시키면서 90도 회전한 점들을 좌표리스트에 추가한다. <br/>
3️⃣ 모든 좌표들이 최종적으로 진화한 좌표들을 100x100 배열에 표시한 후 정사각형의 개수를 세어 출력한다. <br/>
